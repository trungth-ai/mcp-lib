...
| `curl` từ host MCP tới `10.1.0.205:<cổng>` báo **timeout** (không phải "refused"), nhưng `curl`/browser NGAY TRÊN host DSpace (qua `localhost:<cổng>`) lại chạy bình thường | Firewall trên chính **host DSpace** (Windows — không phải `ufw`, phải kiểm Windows Firewall inbound rules) đang chặn LAN khác, chỉ cho `127.0.0.1`. `ufw status` trên host MCP chỉ kiểm được chiều đi ra, không nói lên gì về chiều nhận ở host kia. Kiểm thêm: DSpace/Tomcat có bind đúng `0.0.0.0` (không chỉ `127.0.0.1`) không; nếu có Traefik đứng trước, route hiện tại có đang chặn truy cập LAN trực tiếp (chỉ cho qua domain public) không |
